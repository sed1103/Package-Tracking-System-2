...
/**
 * @brief Function to remove a package from the hash table.
 *
 * This function removes a package from the hash table based on its tracking number.
 *
 * @param trackingNumber The tracking number of the package to remove.
 *
 * @note If the specified tracking number is not found in the hash table, a message will be printed.
 */

void removePackage(char* trackingNumber) {
    int index = hash(trackingNumber);
    ...
    while (current != NULL) {
        if (strcmp(current->package.trackingNumber, trackingNumber) == 0) {
            // Package found, remove it from the linked list
            if (prev == NULL) {
                // The package is at the beginning of the linked list
                hashTable[index] = current->next;
            } else {
                prev->next = current->next;
            }

            // Free memory allocated for the removed package
            free(current);

            printf("Package with tracking number %s removed successfully.\n", trackingNumber);
            return;
        }
        prev = current;
        current = current->next;
